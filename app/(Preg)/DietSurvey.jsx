...
const DietaPersonalizada = () => {
  ...
  const waterIntakeOptions = [1, 2, 3, 4]
  ...
  const renderDietTypeOptions = () => {
    return (
      <FlatList
        ...
        renderItem={({ item }) => (
          <Pressable
            style={[styles.dietOption, selectedDietType === item.value && styles.dietOptionSelected]}
            onPress={() => setSelectedDietType(item.value)}
            android_ripple={{ color: "rgba(0, 208, 120, 0.1)" }}
          >
            <View style={styles.dietOptionHeader}>
              <View style={styles.radioButton}>
                {selectedDietType === item.value && <View style={styles.radioButtonSelected} />}
              </View>
              ...
              />
              <Text style={[styles.dietOptionTitle, selectedDietType === item.value && styles.dietOptionTitleSelected]}>
                {item.label}
              </Text>
            </View>
  ...
  const renderGoalOptions = () => {
    return (
      <FlatList
        ...
        renderItem={({ item }) => (
          <Pressable
            style={[styles.optionCard, selectedGoal === item.value && styles.optionCardSelected]}
          ...
          >
            <View style={styles.optionHeader}>
              ...
              />
              <Text style={[styles.optionCardTitle, selectedGoal === item.value && styles.optionCardTitleSelected]}>
                {item.label}
              </Text>
            </View>
  ...
  const renderActivityOptions = () => {
    return (
      <FlatList
        ...
        renderItem={({ item }) => (
          <Pressable
            style={[styles.optionCard, selectedActivity === item.value && styles.optionCardSelected]}
          ...
          >
            <View style={styles.optionHeader}>
              ...
              />
              <Text style={[styles.optionCardTitle, selectedActivity === item.value && styles.optionCardTitleSelected]}>
                {item.label}
              </Text>
            </View>
            <Text style={styles.optionCardDescription}>{item.description}</Text>
        ...
        <FlatList
          ...
          renderItem={({ item }) => (
            ...
            >
              <View style={styles.checkbox}>
                {selectedValues.includes(item.value) && <View style={styles.checkboxSelected} />}
              </View>
          ...
          ListFooterComponent={
            <>
              ...
              >
                <View style={styles.checkbox}>
                  {(isAllergies ? showOtherAllergyInput : showOtherDislikeInput) && (
              ...
              {(showOtherAllergyInput && isAllergies) || (showOtherDislikeInput && !isAllergies) ? (
                <View style={styles.otherInputContainer}>
                  <TextInput
                    style={styles.otherInput}
                    placeholder={
                      isAllergies 
                        ? "Escribe tu alergia o intolerancia" 
                        : "Escribe el alimento que evitas"
                    }
                    value={isAllergies ? otherAllergy : otherDislike}
                    onChangeText={isAllergies ? setOtherAllergy : setOtherDislike}
                    placeholderTextColor={COLORS.textSecondary}
                  />
                  <Pressable
                    style={styles.addButton}
                    onPress={isAllergies ? addOtherAllergy : addOtherDislike}
              ...
              {selectedAllergies.some((a) => !allergyOptions.some((o) => o.value === a)) && isAllergies && (
                <View style={styles.addedItemsContainer}>
                  {selectedAllergies
                    ...
                    .map((item, index) => (
                      <View key={`added-allergy-${index}`} style={styles.addedItem}>
              ...
              {selectedDislikes.some((d) => !dislikeOptions.some((o) => o.value === d)) && !isAllergies && (
                <View style={styles.addedItemsContainer}>
                  {selectedDislikes
                    ...
                    .map((item, index) => (
                      <View key={`added-dislike-${index}`} style={styles.addedItem}>
                        <Text style={styles.addedItemText}>{item}</Text>
                        <Pressable
  ...
  const renderPreferences = () => {
    return (
      <View style={styles.preferencesContainer}>
        <View style={styles.preferenceSection}>
          ...
          <View style={styles.mealsButtons}>
            {[3, 4, 5, 6].map((num) => (
              <Pressable
                key={num}
                style={[styles.mealButton, mealsPerDay === num && styles.mealButtonSelected]}
              ...
              >
                ...
                />
                <Text style={[styles.mealButtonText, mealsPerDay === num && styles.mealButtonTextSelected]}>{num}</Text>
              </Pressable>
            ))}
          </View>
        </View>
        ...
        <View style={styles.preferenceSection}>
          <Text style={styles.preferenceTitle}>¿Cuánta agua tomas al día?</Text>
          <View style={styles.mealsButtons}>
            {waterIntakeOptions.map((liters) => (
              <Pressable
                key={liters}
                style={[styles.mealButton, waterIntake === liters && styles.mealButtonSelected]}
                onPress={() => setWaterIntake(liters)}
                android_ripple={{ color: "rgba(255, 255, 255, 0.2)" }}
              >
                <MaterialCommunityIcons
                  name="water"
                  size={24}
                  color={waterIntake === liters ? COLORS.text : COLORS.textSecondary}
                />
                <Text style={[styles.mealButtonText, waterIntake === liters && styles.mealButtonTextSelected]}>
                  {liters}L
                </Text>
              </Pressable>
            ))}
        ...
        <View style={styles.preferenceSection}>
          <Text style={styles.preferenceTitle}>Tiempo máximo de preparación</Text>
          <View style={styles.timeButtons}>
            {cookingTimeOptions.map((time) => (
              <Pressable
                key={time}
                style={[styles.timeButton, cookingTime === time && styles.timeButtonSelected]}
              ...
              >
                ...
                />
                <Text style={[styles.timeButtonText, cookingTime === time && styles.timeButtonTextSelected]}>
                  {time} min
                </Text>
              </Pressable>
            ))}
        ...
        return (
          <View style={styles.completionContainer}>
            ...
            <Text style={styles.completionText}>
              ¡Estamos generando tu dieta personalizada basada en tus preferencias!
            </Text>
  ...
  const isNextButtonDisabled = () => {
    return (step === 0 && !selectedGoal) || (step === 1 && !selectedActivity) || (step === 2 && !selectedDietType)
  }
